...
class AttributeCalculator:
    ...
    def calculate(
        ...
        _dl_creation_p36 = (iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0)
        ...
        _sd_c_blk = self._norm(blk_per36, 0.20, 2.8)
        ...
        _dd_c_transition = self._norm(transition_pos, 0.20, 4.5)
        _dd_c_creation = self._norm((iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0), 0.60, 9.5)
        _dd_c_fta = self._norm(fta_rate, 0.12, 0.50)
        _dd_c_size = 0.60 * self._norm(height, 75, 84) + 0.40 * self._norm(weight, 180, 260)
        _dd_c_pop = max(0.0, min(1.0, 0.60 * _dd_c_size + 0.40 * age_phys))
        _dd_c_burst = (
        ...
        _cs_c_slash = self._norm(transition_pos, 0.20, 4.5)
        _cs_c_post = self._norm(post_pos * 36.0 / max(min_pg, 1.0), 0.10, 4.5)
        _cs_c_oreb = self._norm(oreb_pg, 0.30, 4.0)
        _cs_c_size = 0.55 * self._norm(height, 76, 84) + 0.45 * self._norm(weight, 190, 270)

        _cs_raw = (
            0.17 * _cs_c_paint_pct
            + 0.17 * _cs_c_ra_pct
        ...
        _tp_vol = self._norm(self._f(f, "fg3a_per36", 0), 0.5, 11.5)
        _tp_score = 0.68 * _tp_acc + 0.32 * _tp_vol
        ...
        # do not land too low.
        if fg3_pct >= 0.34 and self._f(f, "fg3a_per36", 0) >= 4.0:
            _tp_score += 0.06 * (0.60 * _tp_acc + 0.40 * _tp_vol)

        # Near non-shooters should stay at the low end.
        if self._f(f, "fg3a_per36", 0) < 0.6:
            _tp_score *= 0.25
        ...
        _mr_creation = self._norm((iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0), 0.6, 9.5)
        ...
        _siq_creation = self._norm((iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0), 0.6, 10.0)
        ...
        _ph_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _pf_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _pf_creation = self._norm((iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0), 0.5, 8.0)
        ...
        _pc_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _df_fta_p36 = self._norm(self._f(f, "fta_per36", 0), 1.5, 11.0)
        _df_rim = self._norm(rim_pressure, 0.10, 0.60)
        _df_ra = self._norm(ra_pct, 0.52, 0.76)
        _df_usage = self._norm(usage, 14.0, 35.0)
        _df_creation = self._norm((iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0), 0.6, 10.0)
        _df_pts = self._norm(pts_pg, 8.0, 30.0)
        _df_post = self._norm(post_pos * 36.0 / max(min_pg, 1.0), 0.10, 5.0)
        ...
        _bh_iso_p36 = iso_pos * 36.0 / max(min_pg, 1.0)
        _bh_pnr_p36 = pnr_bh * 36.0 / max(min_pg, 1.0)
        _bh_trans_p36 = transition_pos * 36.0 / max(min_pg, 1.0)
        ...
        _bh_creation = 0.42 * _bh_pnr + 0.33 * _bh_iso + 0.25 * _bh_usage
        _bh_passing = 0.44 * _bh_ast + 0.28 * _bh_ast_pct + 0.28 * _bh_ast_tov
        _bh_security = 0.62 * _bh_tov + 0.38 * _bh_ast_tov
        ...
        _bh_craft = 0.45 * _bh_three + 0.20 * _bh_mid + 0.35 * _bh_iq
        ...
        _swb_transition_p36 = transition_pos * 36.0 / max(min_pg, 1.0)
        _swb_iso_p36 = iso_pos * 36.0 / max(min_pg, 1.0)
        _swb_pnr_p36 = pnr_bh * 36.0 / max(min_pg, 1.0)
        ...
        _h_roll_p36 = pnr_roll * 36.0 / max(min_pg, 1.0)
        _h_cuts_p36 = cuts_pos * 36.0 / max(min_pg, 1.0)
        _h_spot_p36 = spot_up_pos * 36.0 / max(min_pg, 1.0)
        _h_handoff_p36 = handoff_pos * 36.0 / max(min_pg, 1.0)
        ...
        _h_security = 0.80 * _h_tov + 0.20 * _h_ast_tov
        ...
        _pa_pnr_p36 = pnr_bh * 36.0 / max(min_pg, 1.0)
        _pa_handoff_p36 = handoff_pos * 36.0 / max(min_pg, 1.0)
        _pa_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _pa_security = 0.56 * _pa_ast_tov + 0.44 * _pa_tov
        ...
        _piq_pnr_p36 = pnr_bh * 36.0 / max(min_pg, 1.0)
        _piq_iso_p36 = iso_pos * 36.0 / max(min_pg, 1.0)
        _piq_handoff_p36 = handoff_pos * 36.0 / max(min_pg, 1.0)
        _piq_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _pv_pnr_p36 = pnr_bh * 36.0 / max(min_pg, 1.0)
        _pv_iso_p36 = iso_pos * 36.0 / max(min_pg, 1.0)
        _pv_handoff_p36 = handoff_pos * 36.0 / max(min_pg, 1.0)
        _pv_post_p36 = post_pos * 36.0 / max(min_pg, 1.0)
        ...
        _oc_creation_p36 = (iso_pos + pnr_bh) * 36.0 / max(min_pg, 1.0)
        ...
        _oc_security = 1.0 - self._norm(tov_pct, 0.10, 0.19)
        ...
        _pd_value = 100.0 * _pd_raw * (0.82 + 0.18 * _pd_reliability)
        ...
        if is_guard:
            _dc_raw = 0.58 * _dc_poa + 0.18 * _dc_anchor + 0.24 * _dc_stock
        elif is_big:
            _dc_raw = 0.26 * _dc_poa + 0.58 * _dc_anchor + 0.16 * _dc_stock
        else:
            _dc_raw = 0.46 * _dc_poa + 0.34 * _dc_anchor + 0.20 * _dc_stock
        ...
        _str_contact = 0.42 * _str_anchor + 0.26 * _str_post + 0.18 * _str_draw + 0.14 * _str_sdunk
        ...
        _vt_pop = 0.42 * _vt_ddunk + 0.24 * _vt_blk + 0.18 * _vt_oreb + 0.16 * _vt_rim
        ...
        _st_transition = self._norm(transition_pos, 0.5, 5.5)
        _st_cuts = self._norm(cuts_pos, 0.2, 2.7)
        _st_roll = self._norm(pnr_roll, 0.0, 4.0)
