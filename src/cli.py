...
def _build_pipeline(season: str) -> Any:
    ...
    cache_dir = os.environ.get("N2K_CACHE_DIR")
    if not cache_dir:
        cache_dir = "data/cache" if os.path.isdir("data/cache") else ".cache"
    return TendencyPipeline(cache_dir=cache_dir)
...
def _generate_and_save(
    ...
    try:
        ...
        payload["play_style_count"] = result.get("play_style_count", 0)
        payload["play_style_usage_rate"] = result.get("play_style_usage_rate", 0.0)
        payload["play_style_priorities"] = result.get("play_style_priorities", [])
        payload["play_style_weights"] = result.get("play_style_weights", {})
...
def _print_summary(players: list[dict]) -> None:
    ...
    if not players:
        return
    header = f"{'Player':<25} {'Pos':<5} {'Style 1':<22} {'Styles':>6}"
    print("\n" + header)
    print("-" * len(header))
    for p in players:
        name = p.get("player_name", "Unknown")[:24]
        pos = p.get("position", "?")[:4]
        styles = p.get("play_style_priorities", [])
        top_style = styles[0] if styles else "-"
        print(f"{name:<25} {pos:<5} {top_style:<22} {len(styles):>6}")
