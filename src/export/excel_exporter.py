...
# ---------------------------------------------------------------------------
# Attribute Excel export
# ---------------------------------------------------------------------------

# Column order requested by the user — canonical attribute keys.
_ATTRIBUTE_EXPORT_ORDER: tuple[str, ...] = (
    "driving_layup",
    "post_fade",
    "post_hook",
    "post_control",
...
_ATTRIBUTE_DISPLAY_LABELS: dict[str, str] = {
...
def export_team_attributes_excel(
    team_abbr: str,
    team_data: list[dict[str, Any]],
) -> bytes:
    """Export team attributes to Excel bytes.

    Parameters
    ----------
    team_abbr:  Team abbreviation (used in the sheet title).
    team_data:  List of {player_name, position, attributes: {canonical_name: int}}.
    ...
    # Row 1: header — Player, Position, then attribute labels in order
    for col, heading in enumerate(["Player", "Position"], start=1):
        cell = ws.cell(row=1, column=col, value=heading)
        cell.font = header_font
        cell.fill = header_fill
    ...
    for col_idx, attr_key in enumerate(_ATTRIBUTE_EXPORT_ORDER, start=3):
        label = _ATTRIBUTE_DISPLAY_LABELS.get(attr_key, attr_key)
