...
from src.ingest.nba_api_client import NBAApiClient, _strip_accents
...
def _clamp_mid_family_to_parent_band(
    rounded: dict[str, int],
    parent_key: str,
    family_keys: list[str],
    buffer: int = 15,
) -> None:
    """Ensure final rounded mid sub-zones do not exceed parent + buffer."""
    parent_val = max(0, int(rounded.get(parent_key, 0)))
    hard_ceiling = _round_to_5(min(100, parent_val + buffer))
    for key in family_keys:
        rounded[key] = min(max(0, int(rounded.get(key, 0))), hard_ceiling)
...
class TendencyPipeline:
    ...
    def __init__(
        ...
        self._play_styles = PlayStyleScorer()
        self._attributes = AttributeCalculator()
        ...
        # Load attribute ML models if available
    ...
    def generate(self, player_id: int | str, season: str = "2024-25") -> dict[str, Any]:
        ...
        Parameters
        ----------
        player_id: Numeric NBA player ID **or** player name string.
                   If a string is passed it is resolved via search_player().
        ...
        """
        # Resolve name → numeric ID when a string is passed
        if isinstance(player_id, str):
            try:
                player_id = int(player_id)
            except ValueError:
                matches = self.search_player(player_id)
                if not matches:
                    raise ValueError(f"Player '{player_id}' not found")
                name_lower = _strip_accents(player_id.lower())
                exact = next(
                    (m for m in matches if _strip_accents(m.get("full_name", "").lower()) == name_lower),
                    matches[0],
                )
        ...
        try:
            ...
            play_style_count = style_result.target_count
            play_style_usage_rate = style_result.usage_rate
        except Exception as exc:  # noqa: BLE001
            errors.append(f"Play-style scoring error: {exc}")
        ...
        result: dict[str, Any] = {
            ...
            "play_style_count": play_style_count,
            "play_style_usage_rate": play_style_usage_rate,
