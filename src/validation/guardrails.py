...
class Guardrails:
    ...
    def check(self, tendencies: dict[str, float]) -> list[dict[str, Any]]:
        ...
        # 3b. Mid sub-zones should not greatly exceed shot_mid_range.
        # Keep chart-driven shape, but block pathological outputs like
        # shot_mid_range=0 with a single mid lane at 45.
        mid_zone_keys = [
            "shot_mid_left",
        ...
        mid_zone_cap = max(0.0, shot_mid + 15.0)
        mid_zone_adjusted = False
        for key in mid_zone_keys:
            current = float(tendencies.get(key, 0.0))
            if current > mid_zone_cap:
                tendencies[key] = mid_zone_cap
                mid_zone_adjusted = True

        if mid_zone_adjusted:
            violations.append(
                {
                    "rule": "shot_mid sub-zones <= shot_mid_range + 15",
                    "tendency": "shot_mid sub-zones",
                    "value": "family",
                    "expected": f"each <= {mid_zone_cap:.1f}",
                    "action_taken": f"clamped outliers to {mid_zone_cap:.1f}",
                }
            )
