-- Rollback background/action image feature from Supabase.
-- Safe to run multiple times.

BEGIN;

-- Recreate the generation source view without action_image_url.
DROP VIEW IF EXISTS player_generation_source_v1;

CREATE VIEW player_generation_source_v1 AS
WITH stats_union AS (
  SELECT
    p.player_id,
...
-- Remove index/column used by the old action-image pipeline.
DROP INDEX IF EXISTS idx_player_info_action_image_url_nonempty;
ALTER TABLE player_info DROP COLUMN IF EXISTS action_image_url;
...
-- SELECT column_name
-- FROM information_schema.columns
-- WHERE table_name = 'player_generation_source_v1'
-- ORDER BY ordinal_position;
