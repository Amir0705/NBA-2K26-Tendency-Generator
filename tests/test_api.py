...
class TestGenerateByName:
    ...
    def test_response_has_required_fields(self, client):
        ...
        data = resp.json()
        for field in (
            "player_name", "player_id", "position", "team", "season", "tendencies",
            "play_style_count", "play_style_usage_rate", "play_style_priorities", "play_style_weights",
        ):
...
class TestTeamPlayerEndpoint:
    def test_generates_player_on_team(self, client):
        ...
        assert data["player_name"] == "Stephen Curry"
        assert "tendencies" in data
        assert "play_style_priorities" in data
...
class TestTendencyValues:
    ...
    def test_team_tendency_entry_has_all_primjer_fields(self, client):
        ...
        for player in data["players"]:
            assert "play_style_priorities" in player
            assert "play_style_weights" in player
