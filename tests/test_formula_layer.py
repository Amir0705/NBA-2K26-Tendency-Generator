...
class TestFormulaLayerGenerate:
    ...
    def test_rim_runner_big_gets_higher_alley_oop(self, formula):
        rim_runner = _minimal_features("C")
        rim_runner["zone_fga_rate_ra"] = 0.42
        rim_runner["zone_fga_rate_paint"] = 0.18
        rim_runner["height_inches"] = 84
        rim_runner["weight_lbs"] = 250

        stretch_big = _minimal_features("C")
        stretch_big["zone_fga_rate_ra"] = 0.12
        stretch_big["zone_fga_rate_paint"] = 0.12
        stretch_big["height_inches"] = 82
        stretch_big["weight_lbs"] = 230

        rr = formula.generate(rim_runner)
        sb = formula.generate(stretch_big)
        assert rr["alley_oop"] > sb["alley_oop"]

    def test_elite_guard_playmaker_gets_higher_alley_oop_pass(self, formula):
        elite = _minimal_features("PG")
        elite["ast_per36"] = 11.0
        elite["ast_to_tov"] = 2.8
        elite["usg_pct_proxy"] = 0.31
        elite["zone_fga_rate_ra"] = 0.28

        low_creator = _minimal_features("PG")
        low_creator["ast_per36"] = 3.0
        low_creator["ast_to_tov"] = 1.0
        low_creator["usg_pct_proxy"] = 0.15
        low_creator["zone_fga_rate_ra"] = 0.12

        elite_r = formula.generate(elite)
        low_r = formula.generate(low_creator)
        assert elite_r["alley_oop_pass"] > low_r["alley_oop_pass"]
        assert elite_r["alley_oop_pass"] >= 40
...
class TestImprovementPlan:
    ...
    def test_shorter_player_more_post_spin(self, formula):
        ...
        short_sf = dict(_minimal_features("SF"), height_inches=74,
                        ...
                        post_up_possessions=2.0, ast_per36=5.0)
...
_POST_TARGETS = {
    "embiid": {
        "post_up": 45, "post_shimmy_shot": 10, "post_face_up": 30,
        "post_back_down": 35, "post_aggressive_backdown": 25, "shoot_from_post": 25,
        "post_hook_left": 10, "post_hook_right": 10,
        "post_fade_left": 10, "post_fade_right": 10,
        "post_hop_shot": 10, "post_step_back_shot": 10,
        "post_drive": 25, "post_spin": 15, "post_drop_step": 15,
    },
    "wemby": {
        "post_up": 20, "post_shimmy_shot": 5, "post_face_up": 10,
    ...
    "lebron": {
        "post_up": 25, "post_shimmy_shot": 5, "post_face_up": 15,
        "post_back_down": 15, "post_aggressive_backdown": 10, "shoot_from_post": 15,
        "post_hook_left": 5, "post_hook_right": 5,
        "post_fade_left": 10, "post_fade_right": 10,
        "post_hop_shot": 5, "post_step_back_shot": 5,
        "post_drive": 15, "post_spin": 10, "post_drop_step": 10,
    },
    "luka": {
...
def _post_player_features(pos: str, height_inches: int, weight_lbs: int,
                           zpaint: float, zra: float,
                           post_up_possessions: float = 0.0) -> dict:
...
class TestPostCalibration:
    ...
    def test_embiid_post_tendencies(self, formula):
        ...
        feats = _post_player_features("C", 84, 280, zpaint=0.30, zra=0.20, post_up_possessions=4.5)
    ...
    def test_wembanyama_post_tendencies(self, formula):
        ...
        feats = _post_player_features("C", 88, 225, zpaint=0.20, zra=0.15, post_up_possessions=2.0)
    ...
    def test_lebron_post_tendencies(self, formula):
        ...
        feats = _post_player_features("SF", 81, 250, zpaint=0.30, zra=0.20, post_up_possessions=3.0)
    ...
    def test_luka_post_tendencies(self, formula):
        ...
        feats = _post_player_features("PG", 79, 230, zpaint=0.30, zra=0.20, post_up_possessions=1.2)
    ...
    def test_wemby_lower_post_up_than_embiid(self, formula):
        ...
        embiid = _post_player_features("C", 84, 280, zpaint=0.30, zra=0.20, post_up_possessions=4.5)
        wemby = _post_player_features("C", 88, 225, zpaint=0.20, zra=0.15, post_up_possessions=2.0)
