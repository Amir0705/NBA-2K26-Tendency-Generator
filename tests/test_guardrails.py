...
class TestSubZoneParentAwareNormalization:
    ...
    def test_mid_sub_zones_capped_when_parent_too_low(self, guardrails):
        """Mid lane outliers should be capped when shot_mid_range is very low."""
        t = _base_tendencies()
        t["shot_mid_range"] = 0
        t["shot_mid_left"] = 45
        t["shot_mid_left_center"] = 35
        t["shot_mid_center"] = 10
        t["shot_mid_right_center"] = 5
        t["shot_mid_right"] = 0
        violations = guardrails.check(t)
        assert t["shot_mid_left"] <= 15
        assert t["shot_mid_left_center"] <= 15
        assert any("shot_mid sub-zones <= shot_mid_range + 15" in v.get("rule", "")
                   for v in violations)
