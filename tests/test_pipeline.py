...
def pipeline():
    ...
    p._play_styles = PlayStyleScorer()
...
class TestPipelineGenerate:
    ...
    def test_has_required_fields(self, result):
        for field in ("player_id", "season", "position", "tendencies",
                      ...
                      "play_style_count", "play_style_usage_rate", "play_style_priorities",
                      "play_style_weights", "play_style_scores"):
            assert field in result, f"Missing field: {field}"

    def test_play_style_count_matches_usage_tier(self, result):
        usage = float(result["play_style_usage_rate"])
        expected = 4 if usage > 30 else 3 if usage >= 24 else 2 if usage >= 18 else 1
        assert result["play_style_count"] == expected
        assert len(result["play_style_priorities"]) == expected

    def test_play_style_weights_sum_to_one(self, result):
        weights = result["play_style_weights"]
...
class TestMidFamilyClamp:
    def test_clamp_mid_subzones_to_parent_plus_buffer(self):
        t = {
            "shot_mid_range": 5,
            "shot_mid_left": 25,
            "shot_mid_left_center": 20,
            "shot_mid_center": 5,
            "shot_mid_right_center": 5,
            "shot_mid_right": 20,
        }
        _clamp_mid_family_to_parent_band(
            t,
        ...
        assert t["shot_mid_left"] == 20
        assert t["shot_mid_right"] == 20
        assert t["shot_mid_left_center"] <= 20
